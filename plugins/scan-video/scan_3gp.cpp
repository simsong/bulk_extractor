...
static const char _3GP_MAIN_HEADER[ fix_mp4::HEADER_SIZE] = {'f', 't', 'y', 'p'};
static const int _3GP_SUB_HEADER_NO = 5;
static const int _3GP_SUB_HEADER_SIZE = 3;
static const char _3GP_SUB_HEADER[_3GP_SUB_HEADER_SIZE] = {'3', 'g', 'p'};
...
extern "C"
	void  scan_3gp(const class scanner_params &sp,const recursion_control_block &rcb)
{
	if(sp.version != 1){
		cerr << "scan_3gp requires sp version 1; got version " << sp.version << "\n";
		exit(1);
	}

	if(sp.phase == 0){
		sp.info->name  = "3gp";
		sp.info->feature_names.insert("3gp");
		return;
	}
	...
	feature_recorder *recorder = fs.get_name("3gp");
	...
	for(const unsigned char *cc = s; ((cc < (s + l)) && (cc < (s + l - 0x10))); ++cc){
		...
		if( memcmp((cc + fix_mp4::LENGTH_SIZE), _3GP_MAIN_HEADER,  fix_mp4::HEADER_SIZE) == 0){
			if((memcmp((cc +  fix_mp4::HEADER_SIZE + fix_mp4::LENGTH_SIZE), _3GP_SUB_HEADER, (_3GP_SUB_HEADER_SIZE)) == 0)){
				offset = cc - s;
