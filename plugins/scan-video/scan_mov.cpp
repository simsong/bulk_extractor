...
static const char MOV_MAIN_HEADER[ fix_mp4::HEADER_SIZE] = {'f', 't', 'y', 'p'};
static const int MOV_SUB_HEADER_NO = 5;
static const int MOV_SUB_HEADER_SIZE = 2;
static const char MOV_SUB_HEADER[ fix_mp4::HEADER_SIZE] = {'q', 't'};
	...
	std::vector<size_t> &stco, std::vector<size_t> &stsz, std::vector<size_t> &stsc){
		...
		if((last_atom_idx == 9) || (last_atom_idx == 10) || (last_atom_idx == 11)){	// stco, stsz, stsc
			if(stco.size() == stsz.size()){
				...
				if((offset_2_kw  + fix_mp4::LENGTH_SIZE + fix_mp4::LENGTH_OF_ATOM_NAME) >  total_bytes)
					return false;
...
extern "C"
	void  scan_mov(const class scanner_params &sp,const recursion_control_block &rcb)
{

	if(sp.version != 1){
		cerr << "scan_mov requires sp version 1; got version " << sp.version << "\n";
		exit(1);
	}

	if(sp.phase == 0){
		sp.info->name  = "mov";
		sp.info->feature_names.insert("mov");
		return;
	}
	...
	feature_recorder *recorder = fs.get_name("mov");
	...
	for(const unsigned char *cc = s; ((cc < (s + l)) && (cc < (s + l - 0x10))); ++cc){
		...
		if( memcmp((cc + fix_mp4::LENGTH_SIZE), MOV_MAIN_HEADER,  fix_mp4::HEADER_SIZE) == 0){
			if((memcmp((cc +   fix_mp4::HEADER_SIZE + fix_mp4::LENGTH_SIZE), MOV_SUB_HEADER, (MOV_SUB_HEADER_SIZE)) == 0)){
				offset = cc - s;
