...
static void add_find_pattern(const string &pat)
{
...
void scan_find(const class scanner_params &sp,const recursion_control_block &rcb)
{
  ...
  if (scanner_params::PHASE_INIT == sp.phase) {
    for (vector<string>::const_iterator itr(FindOpts.Patterns.begin()); itr != FindOpts.Patterns.end(); ++itr) {
      add_find_pattern(*itr);
    }
    for (vector<string>::const_iterator itr(FindOpts.Files.begin()); itr != FindOpts.Files.end(); ++itr) {
