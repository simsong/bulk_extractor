	...
	for(const unsigned char *cc=sbuf.buf; cc < sbuf.buf+sbuf.pagesize && cc < sbuf.buf + sbuf.bufsize; cc++) {
            ...
            if(record_components && process_component(cc, cc_len, component)) {
                ...
                if(component.compression_method != METHOD_UNCOMPRESSED) {
                    uint8_t *buf = (uint8_t *)calloc(component.uncompressed_size,1);
                    if (buf==nullptr){
                        throw std::bad_alloc();
                    }
                    unpack_buf(cc, cc_len, buf, component.uncompressed_size);

                    /* Create a child sbuf with the updated pos0 for recursive processing */
                    const pos0_t pos0_rar = (pos0 + pos) + "RAR";
                    sbuf_t *child_sbuf = new sbuf_t(pos0_rar, buf, component.uncompressed_size, component.uncompressed_size, 0, false, true, false);
                    std::string carve_name("_");
                    carve_name += component.name;
                    for(auto &it : carve_name){
                        if(it=='/') it = '_';
                    }
                    std::string fn = unrar_recorder.carve(*child_sbuf, carve_name, decode_iso8601(component.iso_timestamp()));
                    sp.recurse( child_sbuf ); // will delete the sbuf_t
                }
            }
	}
    }
