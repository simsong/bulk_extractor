...
int8_t check_mftrecord_signature(size_t offset, const sbuf_t &sbuf) {
    ...
    if (sbuf[offset] == 0x46 && sbuf[offset + 1] == 0x49 &&
        ...
        if (fixup_value != sbuf.get16i(offset + SECTOR_SIZE - 2))
            return 2;
        if (fixup_value != sbuf.get16i(offset + SECTOR_SIZE * 2 - 2))
            return 2;
    ...
    if(sp.phase==scanner_params::PHASE_SCAN){
        ...
        while (offset <= stop-MFT_RECORD_SIZE) {
            ...
            if (result_type == 1) {
                ...
                ntfsmft_recorder->carve_records(sbuf,offset,MFT_RECORD_SIZE,"corrupted_mft");
            }
            else {
            }
