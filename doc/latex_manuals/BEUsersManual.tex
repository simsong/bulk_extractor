...
\end{itemize} 
\bulk 1.4 detects and optimistically decompresses data in ZIP, GZIP, RAR, and Microsoft's Hibernation files. This has proven useful, for example, in recovering email addresses from fragments of compressed files found in unallocated space. \\
...
When the installer file is executed, the installation will begin and show a dialog like the one shown in Figure ~\ref{fig:installer1}.  Most users will not require the 32-bit installation and can un-check that box if disk space is an issue. Click on ``Install'' and the installer will install \bulk on your system and then notify you when it is complete.\\
\begin{figure}
	\center
...
Settable Options (and their defaults):
   ...
   -S hash_alg=md5    Specifies hash algorithm to be used for all hash 
       calculations ()
   ...
   -S zip_max_uncompr_size=268435456    Maximum size of a ZIP uncompressed 
       object (zip)
   ...
   -S gzip_max_uncompr_size=268435456    maximum size for decompressing GZIP 
       objects (gzip)
   ...
   -S opt_max_weird_count=2    Ignore FAT32 entries with more things weird than 
       this (windirs)
   -S opt_last_year=2020    Ignore FAT32 entries with a later year than this (windirs)
